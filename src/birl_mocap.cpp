...
@Modifier Xiaohan Chen , Biomimetic and Intelligent Robotics Lab, Guangdong University of Technology  
@修改者 陈晓瀚，广东工业大学，仿生与智能机器人实验室（BIRL）  
*/
...
This program connects to a NatNet server, receives a data stream, and not writes that data stream
to an ascii file（commented out of code） but publishes that data stream (only RigidBody and Skeleton data actually) by ros topic. 

@rostopic   "/birl/mocap/RigidBody"
@rostopic   "/birl/mocap/Skeletons"
...
Usage [optional]:
    ...
    or 
    rosrun birl_mocap birl_mocap
*/

/*

The code related to the output file is commented out.(File writing routines)
输出文件的相关代码已被注释
*/
...
#include <birl_mocap/NatNetTypes.h>
#include <birl_mocap/NatNetCAPI.h>
#include <birl_mocap/NatNetClient.h>
...
std::unordered_map<int, std::string> MS_names;
std::unordered_map<int, std::string> RB_names;
std::unordered_map<int, std::string> Sk_names;
std::unordered_map<int, std::unordered_map<int, std::string>> Sk_RB_names;

int main(int argc, char *argv[])
{
    ros::init(argc, argv, "mocap_publisher");
    ros::NodeHandle n;
    // ros::Publisher RigidBody_data_pub = n.advertise<geometry_msgs::PoseArray>("/birl/mocap/RigidBody", 100);
    ...
    else
    {
        ...
        for (int i = 0; i < pDataDefs->nDataDescriptions; i++)
        {
            ...
            else if (pDataDefs->arrDataDescriptions[i].type == Descriptor_Skeleton)
            {
                ...
                std::unordered_map<int, std::string> Sk_rb;
                for (int j = 0; j < pSK->nRigidBodies; j++)
                {
                    ...
                    Sk_rb[pRB->ID] = pRB->szName;
                }
                Sk_RB_names[pSK->skeletonID] = Sk_rb;
                Sk_names[pSK->skeletonID] = pSK->szName;
            }
...
void NATNET_CALLCONV DataHandler(sFrameOfMocapData *data, void *pUserData)
{
    ...
    static ros::Rate loop_rate(120);

    static ros::Publisher RigidBody_data_pub = n.advertise<birl_mocap::RigidBodyArray>("/birl/mocap/RigidBody", 100);
    static ros::Publisher Skeletons_data_pub = n.advertise<birl_mocap::SkeletonArray>("/birl/mocap/Skeletons", 100);
    // static ros::Publisher Maker_data_pub = n.advertise<geometry_msgs::PoseArray>("/birl/mocap/Maker", 100);
    ...
    // Rigid Bodies
    birl_mocap::RigidBodyArray RBs_data;
    birl_mocap::RigidBody RB_single;
    ...
    for (i = 0; i < data->nRigidBodies; i++)
    {
        ...
        RBs_data.RigidBody.push_back(RB_single);
        ...
        RBs_data.RigidBody[i].pose.orientation.w = data->RigidBodies[i].qw;
    ...
    RigidBody_data_pub.publish(RBs_data);
    // loop_rate.sleep();
    ROS_INFO("RigidBody_data_pub");

    // Skeletons

    birl_mocap::SkeletonArray Skeletons_data;
    birl_mocap::Skeleton Sk_single;
    printf("Skeletons [Count=%d]\n", data->nSkeletons);
    for (i = 0; i < data->nSkeletons; i++)
    {
        Skeletons_data.Skeleton.push_back(Sk_single);

        sSkeletonData skData = data->Skeletons[i];
        // printf("Skeleton [ID=%d  Bone count=%d]\n", skData.skeletonID, skData.nRigidBodies);
        Skeletons_data.Skeleton[i].name = Sk_names[skData.skeletonID];
        for (int j = 0; j < skData.nRigidBodies; j++)
        {
            Skeletons_data.Skeleton[i].RigidBody.push_back(RB_single);

            sRigidBodyData rbData = skData.RigidBodyData[j];
            Skeletons_data.Skeleton[i].RigidBody[j].name = Sk_RB_names[skData.skeletonID][rbData.ID];
            // printf("Bone %d\t%3.2f\t%3.2f\t%3.2f\t%3.2f\t%3.2f\t%3.2f\t%3.2f\n",
            //        rbData.ID, rbData.x, rbData.y, rbData.z, rbData.qx, rbData.qy, rbData.qz, rbData.qw);

            Skeletons_data.Skeleton[i].RigidBody[j].pose.position.x = rbData.x;
            Skeletons_data.Skeleton[i].RigidBody[j].pose.position.y = rbData.y;
            Skeletons_data.Skeleton[i].RigidBody[j].pose.position.z = rbData.z;
            Skeletons_data.Skeleton[i].RigidBody[j].pose.orientation.x = rbData.qx;
            Skeletons_data.Skeleton[i].RigidBody[j].pose.orientation.y = rbData.qy;
            Skeletons_data.Skeleton[i].RigidBody[j].pose.orientation.z = rbData.qz;
            Skeletons_data.Skeleton[i].RigidBody[j].pose.orientation.w = rbData.qw;
        }
    }
    Skeletons_data_pub.publish(Skeletons_data);
